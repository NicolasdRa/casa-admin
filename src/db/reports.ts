...
export function annualPnl(db: Db, y: string): AnnualPnl {
  ...
  const income = bookings.filter((b) => b.type === "booking").reduce((s, b) => s + b.amountEur, 0);
...
export function multiYearBalance(db: Db, fromYear = "2023") {
  const years = reportYears(db).filter((y) => y >= fromYear);
  let cumulative = 0;
  return years.map((y) => {
...
export function incomeVsExpenseByMonth(db: Db) {
  ...
  for (const b of listBookings(db))
    if (b.type === "booking") bump(month(b.date), "income", b.amountEur);
